...
- llm_judge_faithfulness is the one LLM-backed check. It routes through the
  LLMPort protocol and reuses the pipeline's own critic prompt + bounded
  repair machinery, so with MockProvider it is answered deterministically
  (the prompt carries the "TASK: CRITIC" marker) and with a live provider
  it is a real independent faithfulness judge.
...
def llm_judge_faithfulness(llm: LLMPort, source_text: str, doc: KnowledgeDoc) -> CriticResult:
    """LLM-judge check: score the doc's faithfulness to the source via LLMPort.

    Reuses the pipeline's critic prompt + parse/repair machinery (marker
    "TASK: CRITIC"), so MockProvider answers it deterministically and any
    live provider gives a real second-opinion verdict. Raises PipelineError
    if the judge's output cannot be parsed even after one repair.
    """
    raw = RawDocument(
    ...
    result, _response = run_critic(llm, raw, draft, max_repairs=1)
    return result
